...
class SearchChain:
    ...
    def generate_search_queries(self, query: str, num_queries: int = 3) -> List[str]:
        ...
        generate_prompt = ChatPromptTemplate.fr(
            template=generate_queries_template,
            input_variables=["query", "num_queries"]
        )
        generate_chain = LLMChain(llm=self.llm, prompt=generate_prompt)
        
        try:
            result = generate_chain.run(query=query, num_queries=num_queries)
            # Parse JSON response
